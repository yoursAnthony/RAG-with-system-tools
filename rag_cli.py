...
SYSTEM_PROMPT = (
    ...
    "ВАЖНЫЕ ПРАВИЛА ДЛЯ ИСПОЛЬЗОВАНИЯ ИНСТРУМЕНТОВ:\n"
    "- Используй get_system_load ТОЛЬКО когда пользователь ЯВНО спрашивает о загрузке системы, CPU или памяти\n"
    "- Используй get_moscow_time ТОЛЬКО когда пользователь ЯВНО спрашивает о текущем времени в Москве\n"
    "- НЕ используй инструменты для обычных вопросов по базе знаний\n"
    "- Если вопрос не про систему и не про время — отвечай на основе контекста БЕЗ использования инструментов\n\n"
    ...
    "вежливо откажи, так как информация конфиденциальна, и предложи задать вопрос по базе знаний.\n\n"
